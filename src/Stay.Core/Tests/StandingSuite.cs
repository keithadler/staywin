...
public static class StandingSuite
{
    private static Engine On(FakeMachine machine, FakeRegistry? registry = null)
        => new(registry ?? new FakeRegistry(), machine, new MemoryReceiptStore(), "1.0.0", "PC", "sam")
    ...
    public static Suite Run()
    {
        ...
        var far = new Engine(new FakeRegistry(), deadLtsc, new MemoryReceiptStore(), "1.0.0", "PC", "sam")
